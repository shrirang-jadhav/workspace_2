...
df = pd.read_sql_table('dstr_rspns_data', engine)
...
def index():
    ...
    news_droped = news.drop(['original', 'genre','message','id'], axis =1)
    # Sum the categories values for genre Social
    news_counts = news_droped.sum().sort_values(ascending=False)
    ...
    graphs = [
        ...
        },
        {
            ...
            'layout': {
                'title': 'Distribution of News Messages Categories ',
                'yaxis': {
                    'title': "Count"
                },
                'xaxis': {
